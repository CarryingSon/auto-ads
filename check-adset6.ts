...
async function findAdSet6() {
  ...
  const adAccountId = 'act_609aboroivse'; // Puzzle Pal account
  
  // List ad sets to find "ad set 6"
  console.log('=== Iščem "Ad Set 6" v Puzzle Pal ===\n');
  
  const res = await fetch(
    `https://graph.facebook.com/v21.0/act_609325472611630/adsets?fields=id,name,status&access_token=${accessToken}`
  );
