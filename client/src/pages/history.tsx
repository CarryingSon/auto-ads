...
function getStatusColor(status: string): string {
  ...
  return "bg-primary/10 text-primary border-primary/20 dark:bg-primary/30 dark:text-primary border-primary/40";
}
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-foreground tracking-tight" data-testid="text-page-title">History</h1>
          <p className="text-sm text-muted-foreground">Upload history and performance overview</p>
        </div>
        <Link href="/bulk-ads">
          <Button 
            className="bg-primary hover:bg-primary/90 text-white px-5 h-9 rounded-xl text-xs font-semibold shadow-md shadow-primary/20 hover:shadow-primary/30 transition-all"
      ...
      {isLoading ? (
        <div className="glass-panel rounded-2xl p-12 flex items-center justify-center">
          <Loader2 className="h-8 w-8 animate-spin text-meta" />
        </div>
      ) : (
        <>
          {/* Queue Section */}
          {queueJobs.length > 0 && (
            <div className="glass-panel rounded-xl overflow-hidden" data-testid="section-queue">
              <div className="p-4 pb-3 border-b border-border/50 dark:border-white/5">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-meta text-lg">queue</span>
                    <h3 className="text-sm font-bold text-foreground">Queue</h3>
                    <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 rounded-full bg-primary/10 text-meta text-[10px] font-bold border border-primary/20" data-testid="badge-queue-count">
                      {queueJobs.length}
                    </span>
                  </div>
                  <Button
                ...
                {queueJobs.map((job) => {
                  ...
                  return (
                    <div key={job.id} data-testid={`queue-job-${job.id}`}>
                      ...
                      >
                        <span className={`material-symbols-outlined text-sm text-muted-foreground transition-transform ${isExpanded ? 'rotate-90' : ''}`}>
                          chevron_right
                        </span>
                      ...
                      {isExpanded && (
                        <div className="px-4 pb-4 pt-1 ml-7 mr-4" data-testid={`queue-details-${job.id}`}>
                          <div className="bg-muted dark:bg-white/5 rounded-lg p-4 space-y-3 border border-border/50 dark:border-white/10">
                            {/* Progress bar */}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                              <div>
                                <p className="text-[10px] text-muted-foreground mb-0.5">Ad Sets</p>
                                <p className="text-xs font-medium text-foreground" data-testid={`detail-adsets-${job.id}`}>
                                  {job.completedAdSets || 0} / {job.totalAdSets || 0}
                                </p>
                              </div>
                              <div>
                                <p className="text-[10px] text-muted-foreground mb-0.5">Ads Created</p>
                                <p className="text-xs font-medium text-foreground" data-testid={`detail-ads-${job.id}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <div>
                                <p className="text-[10px] text-muted-foreground mb-0.5">Upload Mode</p>
                                <p className="text-xs font-medium text-foreground">
                                  {job.adUploadMode === "dynamic" ? "Dynamic" : "Single"}
                                </p>
                              </div>
                              <div>
                                <p className="text-[10px] text-muted-foreground mb-0.5">Drive Folder</p>
                            ...
                            {job.logs && (job.logs as string[]).length > 0 && (
                              <div>
                                ...
                                <div className="bg-console text-console-foreground rounded-md p-3 max-h-72 overflow-y-auto border border-console-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="glass-panel p-4 rounded-xl relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
              <div className="flex justify-between items-start mb-1 relative z-10">
                <p className="text-xs font-medium text-muted-foreground">Total Ads Launched</p>
                <span className="material-symbols-outlined text-muted-foreground text-base">file_upload</span>
              </div>
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-foreground drop-shadow-sm" data-testid="text-total-ads">{totalAdsLaunched}</h2>
                <p className="text-[10px] text-muted-foreground">Launched via Auto-ads</p>
              </div>
              <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-primary/10 rounded-full blur-xl group-hover:bg-primary/20 transition-colors" />
            </div>

            <div className="glass-panel p-4 rounded-xl relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
              <div className="flex justify-between items-start mb-1 relative z-10">
                <p className="text-xs font-medium text-muted-foreground">Latest Ads Launched</p>
                <span className="material-symbols-outlined text-muted-foreground text-base">bar_chart</span>
              </div>
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-foreground drop-shadow-sm" data-testid="text-latest-ads">{latestAdsLaunched}</h2>
                <p className="text-[10px] text-muted-foreground">Created in past 30 days</p>
              </div>
            ...
            <div className="glass-panel p-4 rounded-xl relative overflow-hidden group hover:-translate-y-0.5 transition-transform duration-300">
              <div className="flex justify-between items-start mb-1 relative z-10">
                <p className="text-xs font-medium text-muted-foreground">Total Time Saved</p>
                <span className="material-symbols-outlined text-muted-foreground text-base">schedule</span>
              </div>
              <div className="relative z-10">
                <h2 className="text-2xl font-bold text-foreground drop-shadow-sm" data-testid="text-time-saved">
                  {totalTimeSaved} <span className="text-lg font-semibold text-muted-foreground">min</span>
                </h2>
                <p className="text-[10px] text-muted-foreground">Estimated productivity gain</p>
              </div>
          ...
          <div className="glass-panel p-4 rounded-xl">
            <div className="mb-3">
              <h3 className="text-sm font-bold text-foreground">Upload Activity</h3>
              <p className="text-xs text-muted-foreground">Ads launched over time</p>
            </div>
            <div className="h-36 w-full flex items-end gap-3 relative pl-7 pb-5">
              {/* Y-axis labels */}
              <div className="absolute left-0 top-0 h-full flex flex-col justify-between text-[10px] text-muted-foreground pb-5 pr-1">
              ...
              <div className="absolute inset-0 pb-5 pl-7 flex flex-col justify-between pointer-events-none z-0">
                <div className="w-full h-px border-t border-dashed border-input opacity-50" />
                <div className="w-full h-px border-t border-dashed border-input opacity-50" />
                <div className="w-full h-px border-b border-input opacity-50" />
              </div>
              {/* Bars */}
                    ...
                    <div key={idx} className="w-full mx-1 flex flex-col justify-end group cursor-pointer">
                      <div 
                        className={`rounded-t-lg transition-all duration-500 relative ${
                          isActive 
                            ? "bg-primary shadow-[0_0_15px_hsl(var(--primary)/0.4)] group-hover:shadow-[0_0_20px_hsl(var(--primary)/0.6)]" 
                            : "bg-primary/20 group-hover:bg-primary/40"
                        }`}
                        style={{ height: `${barHeight}px` }}
                        data-testid={`bar-chart-${idx}`}
                      >
                        <div className="invisible group-hover:visible absolute -top-7 left-1/2 -translate-x-1/2 bg-foreground text-background text-[10px] px-1.5 py-0.5 rounded pointer-events-none whitespace-nowrap z-20">
                          {day.count}
                        </div>
                      </div>
                      <span className="text-xs text-muted-foreground text-center mt-2">{day.date}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="glass-panel rounded-xl overflow-hidden pb-2">
            <div className="p-4 pb-3 flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-border/50 dark:border-white/5">
              <div>
                <h3 className="text-sm font-bold text-foreground">Upload Batches</h3>
                <p className="text-xs text-muted-foreground">Grouped by upload hour across all ad accounts</p>
              </div>
              <div className="flex items-center gap-3 flex-wrap">
                <Select value={accountFilter} onValueChange={setAccountFilter}>
                  <SelectTrigger 
                    className="w-[160px] h-8 bg-white/60 dark:bg-white/5 border-border/80 dark:border-white/10 rounded-xl text-xs font-medium text-foreground shadow-sm hover:shadow transition-all" 
                ...
                <Select value={mediaFilter} onValueChange={setMediaFilter}>
                  <SelectTrigger 
                    className="w-[140px] h-8 bg-white/60 dark:bg-white/5 border-border/80 dark:border-white/10 rounded-xl text-xs font-medium text-foreground shadow-sm hover:shadow transition-all" 
                ...
                <Button 
                  variant="outline" 
                  className="h-8 bg-white/60 dark:bg-white/5 border-border/80 dark:border-white/10 rounded-xl text-xs font-medium text-foreground shadow-sm hover:shadow hover:bg-white/80 dark:hover:bg-white/10 transition-all gap-1.5"
            ...
            {filteredBatches.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16">
                <span className="material-symbols-outlined text-5xl text-muted-foreground mb-4">history</span>
              ...
              <>
                      ...
                      <div key={idx} data-testid={`row-batch-${idx}`}>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-4 text-xs text-muted-foreground shrink-0 flex-wrap">
                            <div className="flex items-center gap-2">
                              <div className="w-4 h-4 rounded-full bg-primary/10 dark:bg-primary/40 text-primary dark:text-primary flex items-center justify-center text-[9px] font-bold">
                            ...
                            <div className="space-y-2">
                              {batch.jobs.map((job) => {
                                ...
                                return (
                                  <div 
                                    key={job.id} 
                                    className="bg-muted dark:bg-white/5 rounded-lg p-3 border border-border/50 dark:border-white/10"
                                    data-testid={`batch-job-${job.id}`}
                                  >
                                    <div className="flex items-start justify-between gap-3 mb-2.5 flex-wrap">
                                      <div className="min-w-0">
                                        <p className="text-xs font-semibold text-foreground truncate" data-testid={`batch-job-campaign-${job.id}`}>
                                          {job.campaignName || job.driveRootFolderName || "Untitled Campaign"}
                                        </p>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-4 gap-y-2">
                                      <div>
                                        <p className="text-[10px] text-muted-foreground mb-0.5">Creatives</p>
                                        <div className="flex items-center gap-1.5">
                                          {(job.videoAssetCount || 0) > 0 && (
                                            <span className="flex items-center gap-0.5 text-xs font-medium text-foreground" data-testid={`batch-videos-${job.id}`}>
                                              <Film className="h-3 w-3 text-primary" />
                                              {job.videoAssetCount}
                                            </span>
                                          )}
                                          {(job.imageAssetCount || 0) > 0 && (
                                            <span className="flex items-center gap-0.5 text-xs font-medium text-foreground" data-testid={`batch-images-${job.id}`}>
                                      ...
                                      <div>
                                        <p className="text-[10px] text-muted-foreground mb-0.5">Ads Created</p>
                                        <p className="text-xs font-medium text-foreground" data-testid={`batch-ads-${job.id}`}>
                                          {job.completedAds || 0} / {job.totalAds || 0}
                                        </p>
                                      </div>
                                      <div>
                                        <p className="text-[10px] text-muted-foreground mb-0.5">Ad Sets</p>
                                        <p className="text-xs font-medium text-foreground" data-testid={`batch-adsets-${job.id}`}>
                                          {job.totalAdSets || 0}
                                        </p>
                                      </div>
                                      <div>
                                        <p className="text-[10px] text-muted-foreground mb-0.5">Duration</p>
                                        <p className="flex items-center gap-1 text-xs font-medium text-foreground" data-testid={`batch-duration-${job.id}`}>
                ...
                {filteredBatches.length > 10 && (
                  <div className="p-6 pt-4 text-center">
                    <button 
                      className="text-sm text-muted-foreground hover:text-meta dark:text-muted-foreground dark:hover:text-white transition-colors"
